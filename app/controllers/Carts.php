...
class Carts extends Controller
{
  ...
  public function delete() : void
  {
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
      if ($this->cart->deleteItem(intval($_POST['product_id']))) 
        ...
        die("Something went wrong. The item could not be deleted");
    }
  }
  ...
  public function pay()
  {
    if ( $_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['transport-type'] === '0' ||  $_POST['transport-type'] === '5')) {
      $data = [
        'user' => $_SESSION['user_id'],
      ...
      // 3. Register purchase order
        if ($this->order->registerOrder($data))
          ...
          die("The order couldn't be registered");

      // 4. Empty cart
        $this->cart->emptyCart($data['cart']);
      } else die("Not enough cash, stranger");
    } else die("You need to select the transport type");
